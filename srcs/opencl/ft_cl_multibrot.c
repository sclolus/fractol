...
static inline void	ft_set_multibrot_kernel_arg(t_cl_execution_data *cl_data
									, t_fractal_data *fractal_data)
{
	...
	ret |= clSetKernelArg(cl_data->kernel, 5, sizeof(t_complexe)
						, (void*)&c_distance);
	ret |= clSetKernelArg(cl_data->kernel, 6, sizeof(uint32_t)
						, (void*)&fractal_data->degree);
	if (ret != CL_SUCCESS)
				...
				, t_fractal_data *fractal_data, t_cl_execution_data *cl_data)
{
	...
	ft_set_multibrot_kernel_arg(cl_data, fractal_data);
