...
/*   Updated: 2017/07/20 09:47:00 by sclolus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_handler_mouse_motion_newtown(int x, int y, void *param)
{
	...
	coefficient = (t_complexe){ft_double_distance(fractal_data[NEWTOWN].c.min.real_part
												  , fractal_data[NEWTOWN].c.max.real_part) / (3), ft_double_distance(fractal_data[NEWTOWN].c.min.imaginary_part
		, fractal_data[NEWTOWN].c.max.imaginary_part) / 2};
	ft_get_t_fractal_data()[NEWTOWN].z0 = (t_complexe){(double)(x) / (WINDOW_WIDTH) - 1, (double)(y) / (WINDOW_HEIGHT) - 1};
