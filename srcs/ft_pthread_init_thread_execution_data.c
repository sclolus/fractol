...
/*   Updated: 2017/08/19 06:17:20 by sclolus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static void					ft_pthread_init_c_augmentation_rate(
	t_pthread_execution_data *pthread_data, uint32_t index)
{
...
t_pthread_execution_data	*ft_pthread_init_thread_execution_data(
	uint32_t thread_nbr, t_complexe_cadran complexe_cadran)
{
	...
	complexe_delta = (t_complexe)
		{(ft_double_distance(complexe_cadran.min.real_part
	...
	while (i < thread_nbr)
	{
		...
		, {{(WINDOW_WIDTH / thread_nbr) * i, 0}
		, {(WINDOW_WIDTH / thread_nbr) * (i + 1), WINDOW_HEIGHT}}
		, {{complexe_delta.real_part * i + complexe_cadran.min.real_part
		, complexe_cadran.min.imaginary_part}
		, {complexe_delta.real_part * (i + 1) + complexe_cadran.min.real_part
		, complexe_cadran.max.imaginary_part}}
		, {0, 0}, NULL, i, {0}};
